...
use crate::communication::{hid, ctaphid, eventloop};
// use crate::ctaphid;
// use crate::eventloop;
...
fn usb_device() -> usbip_usb_device {
    ...
    b"/frob/bar".iter().enumerate().for_each(|(i, &v)| dev.path[i] = v as u8);
    #[cfg(target_arch = "aarch64")]
    b"1-1".iter().enumerate().for_each(|(i, &v)| dev.busid[i] = v as u8);   
